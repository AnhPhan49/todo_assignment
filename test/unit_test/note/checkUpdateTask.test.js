...
const { checkUpdateTask } = require("../../../src/services/note.service");

describe("note: checkUpdateTask", () => {
  ...
  it("test tick task with userID and task ID correct reference", async () => {
    ...
    const updateTask = await checkUpdateTask(idTask, userId);
  ...
  it("test tick task with userID and task ID incorrect reference", async () => {
    try {
      ...
      await checkUpdateTask(idTask, userId);
    } catch (err) {
